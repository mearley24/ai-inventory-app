...
export default function PasswordVaultScreen({ navigation }: any) {
  ...
  }, [passwords, searchQuery, selectedCategory, user]);
  ...
  const renderPasswordItem = React.useCallback(({ item: password }: { item: PasswordEntry }) => (
    <View>
      <Pressable
  ...
  ), [navigation, copiedId, handleCopyPassword, handleCopyUsername]);
